...
%
function status = model_write_obj_texcoord(model, filename);
%
...
%
    ...
    if has_texcoord
        for i = 1:size(model.mesh.vertices, 1)
            fprintf(fid, 'vt %f %f\n', ...
                        model.texcoord(i, 1), model.texcoord(i, 2));
        end
    end

    % Write faces
    if has_texcoord
        fprintf(fid, 'mtllib textured.mtl\n');
        fprintf(fid, 'usemtl textured\n');
        for i = 1:size(model.mesh.faces, 1)
            fprintf(fid, 'f %d/%d %d/%d %d/%d\n', ...
                        model.mesh.faces(i, 1), model.mesh.faces(i, 1), model.mesh.faces(i, 2), model.mesh.faces(i, 2), model.mesh.faces(i, 3), model.mesh.faces(i, 3));
        end
    else
        for i = 1:size(model.mesh.faces, 1)
            fprintf(fid, 'f %d %d %d\n', ...
